...
struct WendaContentAbstract: HandyJSON {
    ...
    var textH: CGFloat? {
        ...
        let height = (text as NSString).boundingRect(with: CGSize(width: kScreenWidth - 30, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: [.font: UIFont.systemFont(ofSize: 18), .paragraphStyle: paragraphStyle], context: nil).height
...
struct WendaQuestion: HandyJSON {
    ...
    var title: String = ""
    var titleH: CGFloat? {
        return title.textHeight(fontSize: 18, width: kScreenWidth - 30)
    }
...
struct QuestionContent: HandyJSON {
    ...
    var text: String = ""
    var textH: CGFloat? {
        return text.textHeight(fontSize: 16, width: kScreenWidth - 30)
    }
    var large_image_list = [LargeImage]()
}
